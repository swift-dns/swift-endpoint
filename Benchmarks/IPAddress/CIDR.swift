...
let cidrBenchmarks: @Sendable () -> Void = {
    // MARK: - IPv4_CIDR_Create_Then_Check_Is_Loopback_80M

    /// 127.0.0.1
    let ipv4Loopback: IPv4Address = 0x7F_00_00_01
    Benchmark(
        "IPv4_CIDR_Create_Then_Check_Is_Loopback_80M",
        configuration: .init(
            metrics: [.cpuUser],
            warmupIterations: 5,
            maxIterations: 1000
    ...
    // MARK: - IPv4_CIDR_Create_Then_Check_Is_Multicast_80M

    /// 224.0.255.255
    let ipv4Multicast: IPv4Address = 0xE0_00_FF_FF
    Benchmark(
        "IPv4_CIDR_Create_Then_Check_Is_Multicast_80M",
        configuration: .init(
            metrics: [.cpuUser],
            warmupIterations: 5,
            maxIterations: 1000
    ...
    // MARK: - IPv6_CIDR_Create_Then_Check_Is_Loopback_80M

    /// ::1
    let ipv6Loopback: IPv6Address = 0x0000_0000_0000_0000_0000_0000_0000_0001
    Benchmark(
        "IPv6_CIDR_Create_Then_Check_Is_Loopback_80M",
        configuration: .init(
            metrics: [.cpuUser],
            warmupIterations: 5,
            maxIterations: 1000
